...
def read_crawl_delay():
    print("Crawling Start - robot file")
...
def get_staff_details(crawler_delay):
    print("Crawling through the member details")
    page_count=0
    ...
    while True:
        ...
        page_count+=1
    print("Number of SEFA Members ", len(staff_data["name"]))
...
def crawling():
    doc_id=0
    not_found_url=[]
    publication_data_list = {"doc_id": [ ], "title": [ ], "link": [ ], "text": [ ], "published": [ ], "authors": [ ],"content":[]}
    print("Starting Crawling")
    crawler_delay=read_crawl_delay()
    print("Delay - ", crawler_delay)
    staff_data=get_staff_details(crawler_delay)
    print("Member details collected")
   
    print("Publication crawl start")
    page_no = 0
    ...
    while True:
        ...
        page_no += 1
    print("Total publications retrieved is ",len(not_found_url)+ len(page_to_visit) ," , among these ",len(page_to_visit), " publications have authors from SEFA")
    ...
    print("Crawled publications are saved in the jason file")
    author_count = [ ]
